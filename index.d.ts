declare namespace NgTable.Core {
    ...
    interface IDefaultGetData<T> {
         ...
         */
        getOrderByFn(params?: INgTableParams<T>): void
    }
    ...
    interface INgTableParams<T> {
         ...
         * Changes to group will cause `isDataReloadRequired` to return true and the current `page` to be set to 1
         */
        group(group: Grouping<T>): INgTableParams<T>
        /**
    ...
    interface ITableParamsConstructor<T> {
        new (baseParameters?: IParamValues<T>, baseSettings?: ISettings<T>): INgTableParams<T>
    }
}
...
declare namespace NgTable.Browser {
